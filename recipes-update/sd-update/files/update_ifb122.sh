...
# Format reserve partition

while [ ! -e /dev/mmcblk${mmc}p1 ]
do 
	echo "Wait for reserve partition appear."
	echo ""
	sleep 1
done
...
mkfs.vfat /dev/mmcblk${mmc}p2 > /dev/null 2>&1
mkdir -p $KERNEL_PATH
mount -t vfat /dev/mmcblk${mmc}p2 $KERNEL_PATH > /dev/null 2>&1
cp $UPDATE_PATH/$KERNEL_IMAGE $KERNEL_PATH
cp $UPDATE_PATH/imx6ul-ifb122-a2.dtb $KERNEL_PATH
cp $UPDATE_PATH/imx6ul-ifb122-a3.dtb $KERNEL_PATH
sync
umount $KERNEL_PATH
...
mkfs.ext4 -F -E nodiscard /dev/mmcblk${mmc}p3 > /dev/null 2>&1
mkdir -p $ROOTFS_PATH
mount -t ext4 /dev/mmcblk${mmc}p3 $ROOTFS_PATH > /dev/null 2>&1
